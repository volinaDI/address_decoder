function chaoticAddressReplace(text) {
    ...
    //     "шугыла бау-бакша cериктестиги": /([сш][иыу]р?г?р?[аиыо] ?с?л[ао]? ?|ш[иые]гу)?бао?[оу]м?[- ]?ба([кб]ш|шк)а(си)? ?(с )?(с[еи]р[еи]?[кх]?т?.?[аеиы](т[ие])?[сш]т[еиы]ги?|с[еи]р[еи]?кк?( ?та)? ([иэ]?ст[еи]ги)?|с[еи]р[еи]?кк?|с[еи]р[еи]?кк?|сергей( сергей?)|с[еи](кр|р[еи]?)инский?|с[еи]р[еи]к.?( ?и? ?т[ие]с[иея]ги)?|[cт][еи]р[еи]?[кх]?т?[еи]сти[дг]и)?/,
    ...
    // };
    var regex;
    _.keys(recognitionCrutch).forEach(function(elem) {
        regex = new RegExp(recognitionCrutch[elem]);
        text = text.toLowerCase().replace(regex, elem);
    });
